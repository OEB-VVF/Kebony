...
**Status**: White Paper — partially in use (Product Group and Cost Group exist today)

---

## 0. Company-Wide Mandatory Analytical Practice (Mar 2026)

> **DECIDED**: BU and Country analytic plans are **mandatory on all P&L
> journal items** (accounts starting with 6* and 7*) across all Kebony
> entities, effective immediately.

### Applicability Configuration (Odoo)

Each analytic plan is configured with applicability rules:
- **Default Applicability**: Optional
- **P&L accounts (prefix 6, 7)**: Mandatory

This means balance sheet entries (1*–5*) do not require analytics, but
every expense and revenue line must carry both a BU and a Country tag.

### Entity Defaults

| Entity | Country Default | BU Default | Auto-Prefill |
|--------|----------------|------------|--------------|
| **Kebony Inc. (US)** | USA (CAN if partner is Canadian) | 52 (Sales International) | Yes — onchange partner |
| **Kebony Holding (BE)** | COM (Common) | 10 (Leadership) | Yes — onchange partner |
| **Kebony BNV (Production)** | To be configured | To be configured | Phase 2 |

### Auto-Prefill Logic (code in `account_move.py`)

When a partner is selected on an invoice or credit note:
1. System resolves the Country analytic account from the partner's
   `country_id` (CA → CAN, all others → USA for US entity; COM for BE)
2. System resolves the BU analytic account (52 for US, 10 for BE)
3. Both are merged into the `analytic_distribution` dict on each
   invoice line — preserving any existing analytics (Sales Area,
   Product Group, etc.)
4. User can override on any individual line (e.g., change BU to 60
   Marketing for specific lines)

### Backfill (Historical Data)

A **Backfill Analytics** wizard is available under the Accounting Hub
(Kebony menu). It updates posted journal entries retroactively:
- Select company (US or Holding)
- Select date range (default: Jan 1 2026 → today)
- Preview count → Confirm
- Existing analytics are preserved (merge, not replace)
- BU 60 (Marketing) lines are not overwritten with BU 52

### COGS Analytics (Sales Area)

COGS journal entries are auto-generated by Odoo stock moves and do not
carry sales area analytics by default. Rather than intercepting the stock
valuation flow (invasive), the accounting team performs a **monthly EOM
reclassification journal entry** to allocate COGS by sales area:

```
DR 500000  COGS  (analytic: North East)     $X
DR 500000  COGS  (analytic: South East)     $Y
    CR 500000  COGS  (analytic: unallocated) $(X+Y)
```

Source data: margin report grouped by sales area provides the split.

### Freight Accrual (EOM)

The EOM accrual engine now includes **freight** alongside marketing,
royalties, management fees, and sales rep commissions:

| Type | Source | DR Account | CR Account | Partner |
|------|--------|------------|------------|---------|
| Freight | `x_studio_freight_cost` (manual on invoice) | 580560 | 215560 | Invoice customer |

The freight amount is entered manually by the user (carrier estimate).
At EOM, the accrual engine posts the freight accrual with:
- Analytic distribution from the invoice (sales area)
- Partner = invoice customer

When the actual freight bill arrives, the accountant books it against
215560 to clear the accrual. Over/under-accrual is handled the same way
as marketing (see Accounting & Margin Architecture §8).
...
**What must be ready at MVP**: 5 plans created, all analytic account values seeded, `x_kebony_sales_area` field on contacts, Product Group property on products, journal defaults configured.
...
| Plan | Europe (BNV/Holding) | US (INC) | Question it answers | Status |
|------|---------------------|----------|---------------------|--------|
| **Product Group** | Yes (shared) | Yes (shared) | What type of product was sold/produced? | **EXISTS TODAY** |
| **Cost Group (CM Level)** | Yes (shared) | Yes (shared) | Where in the P&L waterfall? | **EXISTS TODAY** |
| **Country** | Yes (75 values) | Yes (USA, CAN, ...) | Where did we sell? | To create |
| **Business Unit (BU)** | Yes (shared) | Yes (shared) | Which department owns the cost/revenue? | To create |
| **Sales Area** | No | Yes (9 US/CAN regions) | Which US/Canada sales territory? | To create |

**Profitability view**: The core management reporting matrix is **Product Group × Cost Group**. This gives margin by product type at each level of the P&L waterfall. The other dimensions (Country, BU, Sales Area) add geographic and organisational drill-down on top.
...
```
  ...
  ┌──────────────────────────────┐      ┌──────────────────────────────┐
  │ Product Grp: KRRS            │      │ Product Grp: KR              │
  │ Cost Group:  COGS-D          │      │ Cost Group:  COGS-D          │
...
## 3. Plan 1: Product Group (EXISTS TODAY)

Tracks all revenue, COGS, and production costs by **product type**. This is the primary axis for product-level profitability analysis. When crossed with Cost Group (CM Level), it produces the margin waterfall per product type.

### Current Practice

Product Group is an **existing analytic plan** used today in Kebony's reporting. It is the coarsest product classification — all products of the same species and processing type are grouped together regardless of profile, length, or thickness.

### Values

| Code | Product Group | What it includes |
|------|---------------|-----------------|
| KSP | Kebony Scots Pine | All Scots Pine products regardless of profile/thickness (1" cladding, 1.5" terrace, 2" pier decking, 2" construction) |
| KRRS | Kebony Radiata Rough Sawn | All Radiata rough sawn products (KRRS 1", KRRS 2", NCC, etc.) |
| KR | Kebony Radiata Machined | All machined Radiata products (KR 1", KR 2", etc.) |
| ACC | Accessories | Non-wood accessories (screws, clips, oil, etc.) |
| OTHER | Other | Other invoiced items not fitting above categories |

> **Note**: Product Group is coarser than Planning Family. All Scots Pine planning families (KSP1C, KSP15T, KSP2PD, KSP2CON) roll up into a single Product Group "KSP". Planning families are production-driven (dryer compatibility); Product Groups are finance-driven (margin reporting).

### Product Property

Product Group will be added as a **Studio property** on `product.template` (field name TBD — likely `x_studio_product_group`). For product-linked transactions (sales, COGS, MO closing), the analytic is derived automatically from the product.

### Why It Must Be a Standalone Analytic Plan

Product Group cannot be a purely computed dimension derived from the product at transaction time because **not all journal entries have a product behind them**. Current practice includes manual bookkeeping entries posted directly to a Product Group analytic:

- **Stock corrections** (manual adjustments, write-offs, NRV)
- **EOM adjustments** (reclass entries, provision adjustments)
- **Overhead allocations** (entries allocated to Product Group proportionally)

These entries don't originate from a sale, delivery, or MO — they are posted directly with a Product Group analytic tag. This is the existing practice and must be preserved.

### Assignment Rules

| Transaction | Product Group Source |
|---|---|
| Customer invoice (revenue + COGS) | From product on the invoice/delivery line (`x_studio_product_group`) |
| Manufacturing Order (MO closing) | From the finished product of the MO |
| Production costs (WIP absorption) | From the finished product of the MO |
| Stock corrections / write-offs | **Manual** — user selects Product Group analytic directly on JE line |
| EOM adjustments / reclass entries | **Manual** — user selects Product Group analytic directly on JE line |
| EOM variance allocation (standard cost) | Proportional to production m³ per Product Group |
| Overhead allocation | Proportional to production m³ per Product Group (or revenue, TBD) |

### MO Closing Requirement

When a Manufacturing Order (Dryer Load) is completed and the finished goods valuation journal entry is posted, the Product Group analytic **must** be set on both the WIP consumption lines and the FG valuation lines. This ensures production costs are traceable to a product type without needing to drill into individual MOs.
...
## 7. Plan 5: Cost Group / Contribution Margin (CM) Level (EXISTS TODAY)
...
|---|---|---|---|
| **Product Group analytics** | ✅ Shared | ✅ Shared | **EXISTS TODAY** — KSP, KRRS, KR, ACC, OTHER |
| **Cost Group (CM Level) analytics** | ✅ | ✅ | **EXISTS TODAY** — P&L waterfall, same structure both entities |
...
## 10. End-of-Month Allocation Workflow
...
## 12. GL Account → Cost Group (CM Level) Mapping
...
## 13. Auto-Posting Rules (Comprehensive)
...
### Rule 0: Product Group — From Product or Manual (EXISTS TODAY)

| Transaction | Source | Method |
|---|---|---|
| Customer invoice (revenue lines) | `product_id.x_studio_product_group` | Auto-set from product on invoice line |
| COGS journal entry (delivery) | `product_id.x_studio_product_group` | Auto-set from product on stock valuation JE |
| MO closing (WIP → FG valuation) | Finished product's `x_studio_product_group` | Auto-set at MO completion posting |
| EOM Accrual JE (US) | Inherited from invoice line's product group | Grouped by Product Group in `action_run_eom_accruals()` |
| Stock corrections / write-offs | **Manual** — user selects directly | No product to derive from |
| EOM adjustments / reclass entries | **Manual** — user selects directly | No product to derive from |
| Overhead / unabsorbed costs | Allocated proportional to production m³ per Product Group | EOM allocation |

> **Key**: Product Group is the only analytic derived from the **product** (not the customer, team, or GL account) for product-linked transactions. For manual entries (stock corrections, EOM adjustments), the user selects the Product Group directly — this is existing practice.
...
## 14. Odoo 19 Implementation Notes
...
1. **Product Group** plan — **EXISTS** — analytic plan with values KSP, KRRS, KR, ACC, OTHER
2. **Cost Group (CM Level)** plan — **EXISTS** — analytic plan with P&L waterfall values
3. Create 3 new plans: **Country**, **BU**, **Sales Area** (US only)
4. Create analytic accounts for each value in the new plans
5. Configure default analytics on partners, sales teams, and journals where possible
6. Sales Area plan restricted to Kebony Inc. (or simply left unused in European entities)
7. Set up default analytic accounts on stock journals (BU=Operations, CM=COGS-D or COGS-I)
8. Add `x_kebony_sales_area` field on `res.partner` (Many2one to Sales Area analytic account)
9. Add `x_studio_product_group` property on `product.template` via Studio (Many2one to Product Group analytic account)

### What Needs Custom Code

| Need | Approach | Priority |
|---|---|---|
| Auto-assign Product Group on revenue + COGS lines | From product's `x_studio_product_group` → analytic distribution on JE lines | P1 |
| Auto-assign Product Group at MO closing | From finished product → WIP consumption + FG valuation JE lines | P1 |
...
## 15. Decisions & Design Still To Be Made
...
Priority items to enable margin-per-product and P&L waterfall reporting:

| # | Task | Entity | Priority | Dependencies | Status |
|---|---|---|---|---|---|
| 0a | **Product Group plan** — exists in Odoo | All | P0 | None | ✅ EXISTS |
| 0b | **Cost Group (CM Level) plan** — exists in Odoo | All | P0 | None | ✅ EXISTS |
| 1 | **Data seeding**: create 3 new plans (Country, BU, Sales Area) + all analytic account values | All | P0 | None | |
| 1b | **Add `x_studio_product_group`** property on `product.template` via Studio | All | P0 | None | |
| 2 | **Add `x_kebony_sales_area`** on `res.partner` | INC | P0 | Plan 1 done | |
| 3 | **Auto-post Product Group on revenue + COGS lines** | All | P1 | Task 1b | |
| 4 | **Auto-post Product Group at MO closing** (WIP + FG valuation JE) | BNV | P1 | Task 1b | |
| 5 | **Auto-post Country on revenue lines** | All | P1 | Plan 1 done | |
| 6 | **Auto-post Sales Area on revenue lines (US)** | INC | P1 | Tasks 1-2 | |
| 7 | **Auto-post Country + Sales Area on COGS lines** | All / INC | P1 | Tasks 5-6 | |
| 8 | **Auto-post CM Level = REV on revenue** | All | P1 | Plan 1 done | |
| 9 | **Auto-post CM Level on EOM accruals** (MKT, SALES, CORP) | INC | P2 | Plan 1 done | |
| 10 | **BoM-based COGS decomposition** (COGS-D vs COGS-I) — BNV only | BNV | P2 | BoMs validated | |
| 11 | **BU auto-assignment** from sales team + journal defaults | All | P2 | BU plan done | |
| 12 | **EOM unabsorbed cost allocation wizard** | BNV | P3 | Tasks 1-11 | |
| 13 | **EOM country distribution wizard** | BNV | P3 | Task 12 | |
